...
- Obtain a real I2V export for wan22_i2v: the provided export was a T2V graph, so the verified template merges its structure with WanImageToVideo (DECISIONS #4). Smoke-verified live, but re-verify if a genuine I2V export lands. The T2V graph itself (as a T2V template) is unported.
